...
class jeu extends Program{
    ...
    int affichLgth = 0;
    CSVFile bossCSV = loadCSV("../ressources/boss.csv");
    ...
    void printMenu (String page) {
        File menutxt = newFile("../ressources/" + page + ".txt"); 
        while(ready(menutxt)) {
    ...
    void triLeaderboard() {
        ...
        saveCSV(leaderboard, "../ressources/leaderboard.csv");
        leaderboardCSV = loadCSV("../ressources/leaderboard.csv");
    }
    ...
    void testcalulOPeration(){
        assertEquals(calcOperation(1,'+',2),3);
        assertEquals(calcOperation(1,'*',2),2);
        assertEquals(calcOperation(1,'-',2),-1);
        assertEquals(calcOperation(4,'/',2),2);
        assertEquals(calcOperation(4,'%',8),4);
    }
    ...
    String[] initPrintBoss (String boss) {
        File asciiArt = newFile("../ressources/" + boss + ".txt"); 
        int i = 0;
        ...
        File asciiArt2 = newFile("../ressources/" + boss + ".txt"); 
        int j = 0;
    ...
    void testsignEstUtilisable(){
        inventaire inv = NewInvVide();
        ajouterSigne('+',inv);
        assertTrue(signeEstUtilisable(inv,'+'));
        assertFalse(signeEstUtilisable(inv,'*'));
    }
    ...
    void testnombreEstUtilisable(){
        inventaire inv = NewInvVide();
        ajouterNombre(6,inv);
        assertTrue(nombreEstUtilisable(inv,6));
        assertFalse(nombreEstUtilisable(inv,4));
    }
